...
import { saveGlobalConfig, saveProjectSettings, loadProjectSettings } from "../config.js";
...
export async function loginCommand(opts: { baseUrl?: string; apiKey?: string }) {
  const baseUrl = (opts.baseUrl ?? (await prompt("base url", ""))).replace(/\/+$/, "");
  const apiKey = opts.apiKey ?? (await prompt("api key"));
  if (!baseUrl || !apiKey) {
    throw new Error("base url and api key are required");
  }
  const path = saveGlobalConfig({ baseUrl, apiKey });
